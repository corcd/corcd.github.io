---
layout:       post
title:        "在树莓派3上搭建 Apache & Nginx 服务器"
...
千呼万唤的实验室经费终于落实了下来，可以说是迫不及待地购入了时下流行的开发板---树莓派3b。今天作为“硬盘”的 tf 卡万分期盼之中到了，不用多说，马上就开始捣鼓我们的莓莓同学。

# 对树莓派进行初始配置

树莓派3b 较之前作树莓派2代升级了不少，原生板载 WiFi 和蓝牙，可以说对于第一次配置来说，不需要硬连接 PC，直接可以通过 WiFi 局域网远程 SSH 连接配置，省去了 PC 端口配置的步骤，省时省力方便了很多。

当然，树莓派在一开始时是没有匹配连接本地的 WiFi 的，还是需要外部配置。烧录系统的步骤就不累赘了（其实是 tf 卡买来带了系统），不过方法不同版本都大同小异，树莓派官网上有详细的教程。在烧录好系统之后，将 tf 卡插入电脑读卡器，就可以开始 WiFi 的预配置。
