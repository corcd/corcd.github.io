...
### 函数的扩展
##### 箭头函数
在函数体内只包含一条语句时，支持隐式返回
> 返回对象时应将大括号括在括号中避免产生歧义（防止被解析为函数体）

注意：
- 箭头函数中的 this 的作用域继承自执行上下文，本身不绑定 this，所以 this 的值将在调用堆栈中查找 --> 箭头函数不适合作为对象方法
- 箭头函数在作为创建构造函数实例化对象时会抛出 TypeError，不适合这么使用，在不需要动态上下文时使用常规函数
- 事件监听器上也不适合箭头函数，因为 DOM 事件侦听器会自动将 this 与目标元素绑定，当依赖 this 时需要常规函数
```
...
### Class
> class 实现并没有对JS 底层进行修改，仅为原型继承的语法糖
##### 定义
